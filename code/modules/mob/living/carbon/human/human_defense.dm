		...
		if(src.stat == DEAD)
			if(prob(30) && (src.timeofdeath + 1800 > world.time) && !(NOCLONE in src.mutations))
				...
				src.visible_message( \
					...
					"\red <strong>You feel the tearing pain as the life returns to your body!</strong>", \
...
//		return 0
	...
	if((stat < 2) && (!(paralysis + stunned + weakened)))
		if(!(user.dir == src.dir))		//To prevent blocking with ass.

			if(istype(r_hand, /obj/item) && (zone_sel.selecting == target_zone || r_hand.IsShield()))
				if(prob(r_hand:chance_parry))
					visible_message("\red \b [name] deftly parries the attack with [r_hand.name]!")
					if(prob(I.chance_hardblock))
						visible_message("\red \b The [r_hand.name] flies out of [name]'s hands!")
						var/obj/item/R = r_hand
						drop_r_hand()
						R.throw_at(get_step(R,pick(cardinal)),R.throw_range, R.throw_speed, src)
					return

			if(istype(l_hand, /obj/item) && (zone_sel.selecting == target_zone || l_hand.IsShield()))
				if(prob(l_hand:chance_parry))
					visible_message("\red \b [name] deftly parries the attack with [l_hand.name]!")
					if(prob(I.chance_hardblock))
						visible_message("\red \b The [l_hand.name] flies out of [name]'s hands!")
						var/obj/item/R = l_hand
						drop_l_hand()
						R.throw_at(get_step(R,pick(cardinal)),R.throw_range, R.throw_speed, src)
					return
...
//		else			user.update_inv_r_hand()	//removed because weapons don't have on-mob blood overlays
		...
		switch(hit_area)
			...
			if("chest")
				modifier_knockdown = 0.9
				if(bloody)
					bloody_body(src)
			if("right arm","right hand")
				modifier_disarm = 2
				drop_hand = 1
			if("left arm","left hand")
				modifier_disarm = 2
				drop_hand = 2
			if("groin")
			if("right leg","left leg","right foot","left foot")
				modifier_knockdown = 1.5

		if(prob(I.chance_disarm * modifier_disarm))
			if(drop_hand == 1 && src.r_hand)
				src.visible_message("\red \b [src.name] drops their [src.r_hand.name]!")
				src.drop_r_hand()
			else if(drop_hand == 2 && src.l_hand)
				src.visible_message("\red \b [src.name] drops their [src.l_hand.name]!")
				src.drop_l_hand()

		if(prob(I.chance_knockdown * modifier_knockdown))
			apply_effect(6, WEAKEN, armor)
			visible_message("\red <B>[src] has been knocked down!</B>")

		if(prob(I.chance_knockout * modifier_knockout))
			apply_effect(20, PARALYZE, armor)
			visible_message("\red <B>[src] has been knocked unconscious!</B>", "", "\red You hear someone fall.")
