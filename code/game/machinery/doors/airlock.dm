...
/obj/machinery/door/airlock/glass_command/goon
	icon = 'icons/obj/doors/door_bridge_goon.dmi'
...
/obj/machinery/door/airlock/Topic(href, href_list, var/nowindow = 0)
	...
	if((istype(usr, /mob/living/silicon) && src.canAIControl()) || (istype(usr.equipped(), /obj/item/device/hacktool/) && (src.canSynControl())))
		//AI
	...
	if (src.synHacking==0)
		...
		spawn(20 * tick_multiplier)
			...
			for (var/mob/living/silicon/ai/aiPlayer in world)
				if (aiPlayer.stat != 2)
					switch(alertoption)
						if(3) aiPlayer.triggerAlarm("AirlockHacking", src.loc.loc, cameras)
						if(2) aiPlayer.triggerAlarm("AirlockHacking", src.loc.loc)
						if(1) aiPlayer.triggerAlarm("AirlockHacking")
...
/*
/obj/machinery/door/airlock/execute_command(PacketParts)
	switch(command)
		if("ELEC")
*/
