...
/obj/machinery/computer/communications/attack_hand(var/mob/user as mob)
	...
	if (emergency_shuttle.online && emergency_shuttle.location==0)
		...
		if(evac_type == "pods")
			dat += "<B>Escape [capitalize(evac_name)]</B>\n<BR>\nETA: [timeleft / 60 % 60]:[add_zero(num2text(timeleft % 60), 2)]<BR>"
		else
	...
	switch(src.state)
		...
		if(STATE_CALLSHUTTLE)
			dat += "Are you sure you want to launch the [evac_name]? \[ <A HREF='?src=\ref[src];operation=callshuttle2'>OK</A> | <A HREF='?src=\ref[src];operation=main'>Cancel</A> \]"
		if(STATE_CANCELSHUTTLE)
			dat += "Are you sure you want to abort the [evac_name] launch? \[ <A HREF='?src=\ref[src];operation=cancelshuttle2'>OK</A> | <A HREF='?src=\ref[src];operation=main'>Cancel</A> \]"
...
/obj/machinery/computer/communications/proc/interact_ai(var/mob/living/silicon/ai/user as mob)
	...
	switch(src.aistate)
		...
		if(STATE_CALLSHUTTLE)
			dat += "Are you sure you want to launch the Escape [capitalize(evac_name)]? \[ <A HREF='?src=\ref[src];operation=ai-callshuttle2'>OK</A> | <A HREF='?src=\ref[src];operation=ai-main'>Cancel</A> \]"
		if(STATE_MESSAGELIST)
			dat += "Messages:"
			for(var/i = 1; i<=src.messagetitle.len; i++)
		...
		if(STATE_STATUSDISPLAY)
			dat += "Set Status Displays<BR>"
			dat += "\[ <A HREF='?src=\ref[src];operation=setstat;statdisp=blank'>Clear</A> \]<BR>"
			dat += "\[ <A HREF='?src=\ref[src];operation=setstat;statdisp=shuttle'>[capitalize(evac_name)] ETA</A> \]<BR>"
			dat += "\[ <A HREF='?src=\ref[src];operation=setstat;statdisp=message'>Message</A> \]"
			dat += "<ul><li> Line 1: <A HREF='?src=\ref[src];operation=setmsg1'>[ stat_msg1 ? stat_msg1 : "(none)"]</A>"
			dat += "<li> Line 2: <A HREF='?src=\ref[src];operation=setmsg2'>[ stat_msg2 ? stat_msg2 : "(none)"]</A></ul><br>"
			dat += "\[ Alert: <A HREF='?src=\ref[src];operation=setstat;statdisp=alert;alert=default'>None</A> |"
...
/proc/call_shuttle_proc(var/mob/user)
	...
	if(sent_strike_team == 1)
		user << "Centcom will not allow the [evac_name] to be launched. Consider all contracts terminated."
		return

	if(world.time < 6000) // Ten minute grace period to let the game get going without lolmetagaming. -- TLE
		user << "The escape [evac_name] are refueling. Please wait another [round((6000-world.time)/600)] minutes before trying again."
		return

	if(emergency_shuttle.direction == -1)
		user << "The escape [evac_name] may not be launched in such a short time after canceling."
		return

	if(emergency_shuttle.online)
		user << "The escape [evac_name] launch has already been initiated."
		return
	...
	log_game("[key_name(user)] has launched the [evac_name].")
	message_admins("[key_name_admin(user)] has launched the [evac_name].", 1)
	if(evac_type == "pods")
		a.autosay("Alert: The escape [evac_name] launch is taking place. It will be in [round(emergency_shuttle.timeleft()/60)] minutes.", "Escape Computer")
	else
...
/proc/init_shift_change(var/mob/user, var/force = 0)
	...
	if(emergency_shuttle.direction == -1)
		user << "The [evac_name] may not be called in such a short time after canceling."
		return
	...
	if(!force)
		if(emergency_shuttle.deny_shuttle)
			user << "Centcom permits the escape [evac_name] launch. Please try again later."
			return

		if(sent_strike_team == 1)
			user << "Centcom permits the escape [evac_name] launch. Consider all contracts terminated."
			return

		if(world.time < 54000) // 30 minute grace period to let the game get going
			user << "The [evac_name] may not be called at the beginning of the shift. Please wait another [round((54000-world.time)/600)] minutes before trying again."//may need to change "/600"
			return
	...
	log_game("[key_name(user)] has launched the [evac_name].")
	message_admins("[key_name_admin(user)] has launched the [evac_name].", 1)
	captain_announce("A crew transfer has been initiated. The escape [evac_name] will be launched in [round(emergency_shuttle.timeleft()/60)] minutes.")
...
/proc/cancel_call_proc(var/mob/user)
	...
	if(emergency_shuttle.direction != -1 && emergency_shuttle.online) //check that shuttle isn't already heading to centcomm
		...
		log_game("[key_name(user)] has canceled the [evac_name] launch.")
		message_admins("[key_name_admin(user)] has canceled the [evac_name] launch.", 1)
	return
...
/obj/machinery/computer/communications/Destroy()
	...
	log_game("All the AIs, comm consoles and boards are destroyed. [capitalize(evac_name)] launch started.")
	message_admins("All the AIs, comm consoles and boards are destroyed. [capitalize(evac_name)] launch started.", 1)
	if(evac_type == "pods")
		a.autosay("Alert: The escape [evac_name] are being launched. They will launch in [round(emergency_shuttle.timeleft()/60)] minutes.", "Escape Computer")
	else
		a.autosay("Alert: The emergency shuttle has been called. It will arrive in [round(emergency_shuttle.timeleft()/60)] minutes.", "Escape Computer")
	world << sound('sound/AI/shuttlecalled.ogg')
