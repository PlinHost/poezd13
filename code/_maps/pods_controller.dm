...
datum/shuttle_controller/proc/recall()
	...
	if(direction == 1)
		...
		if(alert == 0)
			if(timeleft >= 600)
				return
			a.autosay("The escape [evac_name] launch has been canceled.", "Escape Computer")
...
datum/shuttle_controller/emergency_shuttle/process()
	...
	if((online) && (timeleft() < last60) && (direction == 1))
		if(timeleft > 60)
			a.autosay("[round(timeleft()/60,1)] minutes until escape [evac_name] launch.", "Escape Computer")
			if(timeleft() - 60 > 60)
				last60 = timeleft() - 60
			else
				last60 = 60
		else if(timeleft > 30)
			a.autosay("[round(timeleft(),1)] seconds until escape [evac_name] launch.", "Escape Computer")
			if(timeleft() - 10 > 10)
				last60 = timeleft() - 10
			else
				last60 = timeleft() - 1
			...
			if(timeleft() > 0)
				...
				a.autosay("Escape [evac_name] launched.", "Escape Computer")
			last60 = timeleft() - 1
			...
			if(direction == 2)
				...
				/* --- Shuttle has arrived at Centrcal Command --- */
					...
					*/
					...
					a.autosay("The Escape [capitalize(evac_name)] have docked with the Central Command.", "Escape Computer")
