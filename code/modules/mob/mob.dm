...
/mob/proc/print_flavor_text()
	if(flavor_text && flavor_text != "")
		var/msg = sanitize_uni(text2list2text(flavor_text, "\n", " "))
		if(lentext(msg) <= 40)
			return "\blue [msg]"
		else
...
/mob/Topic(href, href_list)
	...
	if(href_list["flavor_more"])
		usr << browse(text("<HTML><HEAD><TITLE>[]</TITLE></HEAD><BODY><TT>[]</TT></BODY></HTML>", name, sanitize_uni(text2list2text(flavor_text), "\n", "<BR>")), text("window=[];size=500x200", name))
		onclose(usr, "[name]")
...
/mob/proc/Paralyse(amount)
	if(status_flags & CANPARALYSE)
		if(amount && !paralysis)
			visible_message("<font color='blue'><b>[src] loses consciousness.</b></font>", "<font color='red'><b>You lose consciousness.</b></font>")
...
/mob/proc/AdjustParalysis(amount)
	if(paralysis && (paralysis + amount <= 0))
		visible_message("<font color='blue'><b>[src] regains consciousness.</b></font>", "<font color='blue'><b>You regain consciousness.</b></font>")
	if(status_flags & CANPARALYSE)
...
/mob/proc/UpdateLuminosity()			//Не уверен, что требуется, но на всякий случай спиздил с ферна.
	LuminosityRed = max(LuminosityRed, 0)
	LuminosityGreen = max(LuminosityGreen, 0)
	LuminosityBlue = max(LuminosityBlue, 0)
