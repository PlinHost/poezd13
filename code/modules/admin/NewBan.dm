...
/proc/load_bwhitelist()
	...
	if (bwhitelist==list(  ))
		...
		log_admin("Loaded [bwhitelist.len] ckeys in whitelist.")
		world.log << "Loaded [bwhitelist.len] ckeys in whitelist."
		return
...
/proc/add_to_bwhitelist(var/K)
	establish_db_connection()
	if(!dbcon.IsConnected())
		log_admin("Failed to connect to whitelist. Error: [dbcon.ErrorMsg()]")
		world.log << "Failed to connect to whitelist. Error: [dbcon.ErrorMsg()]"
		return
	var/DBQuery/query = dbcon.NewQuery("INSERT INTO whitelist VALUES('[K]')")
	query.Execute()
	log_admin("Added [K] to whitelist.")
	load_bwhitelist()
	return
...
/client/proc/add_p_to_bwhitelist()
	...
	if(added_key)
		if(!check_bwhitelist(added_key))
			...
			log_admin("[usr] attempted to add [added_key] to whitelist.")
		else
